...
from src.Eye_Dector_Module import EyeDetector as EyeDet
...
"""
주어진 코드는 카메라 보정(camera calibration)을 위해 얻은 카메라 행렬(camera matrix) 및 왜곡 계수(distortion coefficients)를 나타냅니다.
카메라 보정은 카메라의 왜곡을 보정하여 실제 세계의 물체를 정확하게 투영하기 위해 필요한 프로세스입니다.
카메라 행렬 (Camera Matrix):
camera_matrix는 3x3 행렬로, 카메라의 내부 파라미터를 정의합니다.
행렬 내의 요소는 다음과 같습니다:
[899.12150372, 0., 644.26261492]: x 축에 대한 초점 거리 및 중심 좌표입니다.
[0., 899.45280671, 372.28009436]: y 축에 대한 초점 거리 및 중심 좌표입니다.
[0, 0, 1]: 정규화 및 이미지 평면까지의 거리를 나타내는 행입니다.

왜곡 계수 (Distortion Coefficients):
dist_coeffs는 왜곡 계수를 나타내는 1x5 행렬입니다.
행렬 내의 요소는 다음과 같습니다:
[-0.03792548, 0.09233237, 0.00419088, 0.00317323, -0.15804257]: 방사 왜곡(radial distortion) 및 접선 왜곡(tangential distortion) 계수입니다.
이러한 내부 파라미터와 왜곡 계수는 카메라 모델을 정의하는 데 사용되며, 이미지에서 발생하는 왜곡을 보정하여 정확한 세계 좌표로의 변환을 가능하게 합니다. 이러한 파라미터는 주로 카메라 캘리브레이션(calibration) 프로세스를 통해 획득되며, 캘리브레이션은 특정 환경에서 카메라의 성능을 향상시키기 위해 수행됩니다.
"""
...
def main():
	...
	while True:  # infinite loop for webcam video capture
		...
		if args.show_proc_time:
			cv2.putText(frame, "PROC. TIME FRAME:" + str(round(proc_time_frame_ms, 0)) + 'ms', (10, 430),
