...
pygame.mixer.init()
sound_siren = pygame.mixer.Sound("./sound/siren.wav")
sound_drowsy_warning = pygame.mixer.Sound("./sound/drowsy_warning.wav")
sound_drowsy_detection = pygame.mixer.Sound("./sound/drowsy_detection.wav")
sound_calling = pygame.mixer.Sound("./sound/calling.wav")
sound_smoking = pygame.mixer.Sound("./sound/smoking.wav")
sound_texting = pygame.mixer.Sound("./sound/texting.wav")
...
# == YOLO ==============================================================================================================
# Load a model
model = YOLO('/Users/bagsangbeom/PycharmProjects/DMS_project/weights/best.pt')  # build a new model from YAML
# object classes
...
cap.set(cv2.CAP_PROP_FPS, 15)  # 프레임 설정
...
while True:
    ...
    if frame is not None:
        frame = imutils.resize(flipped_frame, width=800)
        ...
        frame = yolo_pred(frame, results, CONFIDENCE_THRESHOLD)


        # == cv2.putText 설정 ===========================================================================================
        # 화면에 fps 표시
        cv2.putText(face_mesh_frame, f'FPS: {int(fps)}',(10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, conf.TEXT_COLOR, 1, lineType=cv2.LINE_AA)
        # 화면에 CPU 사용률 표시
        cv2.putText(face_mesh_frame, f'CPU Usage: {cpu_usage}%', (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, conf.TEXT_COLOR, 1, lineType=cv2.LINE_AA)


        # 눈 : 상태 화면에 표시
        cv2.putText(face_mesh_frame, f'{facial_processor.eyes_status}', (10, 240),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, conf.TEXT_COLOR, 2, lineType=cv2.LINE_AA)

        # 눈 : 종횡비 화면에 표시
        cv2.putText(face_mesh_frame, "Left EAR {:.2f}".format(leftEAR), (10, 280),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, conf.TEXT_COLOR, 1, lineType=cv2.LINE_AA)
        cv2.putText(face_mesh_frame, "Right EAR {:.2f}".format(rightEAR), (10, 300),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, conf.TEXT_COLOR, 1, lineType=cv2.LINE_AA)
        ...
        cv2.putText(face_mesh_frame, text, (460, text_y),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, conf.TEXT_COLOR, 1)

        # capture, recoding 상태 화면에 텍스트 표시
        cv2.putText(face_mesh_frame, capture_status_text, (478, text_y + 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, conf.TEXT_COLOR, 1)
        cv2.putText(face_mesh_frame, recording_status_text, (485, text_y + 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, conf.TEXT_COLOR, 1)
