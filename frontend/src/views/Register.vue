    ...
    methods: {
        submitSignup() {
            if (!this.user.nickname || !this.user.email || !this.user.password) {
                ...
                return; // 필수 필드가 비어있다면 함수 종료
            }

            if (!this.user.agreeTerms) {
                alert("Please agree to the terms and conditions.");
                return; // 약관 동의가 필요
            }
            console.log("Sending data to backend:", this.user);
            axios.post('http://localhost:5000/api/auth/signup', this.user)
                .then(response => {
        ...
        toggleAgreeTerms() {
            this.user.agreeTerms = !this.user.agreeTerms;
            console.log("Checkbox State:", this.user.agreeTerms); // 상태 로그 출력
        }
    },
    created() {
