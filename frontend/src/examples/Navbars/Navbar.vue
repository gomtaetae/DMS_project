      ...
      <div class="mt-2 collapse navbar-collapse mt-sm-0 me-md-0 me-sm-4" id="navbar">
        <div class="pe-md-3 d-flex align-items-center">
          <div class="input-group">
            ...
            <input type="text" class="form-control" placeholder="여기에 입력하세요..." />
          </div>
        </div>
        <ul class="navbar-nav justify-content-end">
          <li class="nav-item d-flex align-items-center">
            <a v-if="isLoggedIn" @click="logout" class="nav-link text-white">
              <i class="fa fa-sign-out-alt me-sm-2"></i>
              로그아웃
            </a>
            <router-link v-else :to="{ name: 'Signin' }" class="nav-link text-white">
              <i class="fa fa-sign-in-alt me-sm-2"></i>
              로그인
            </router-link>
          </li>
          <li v-if="isLoggedIn" class="nav-item d-flex align-items-center">
            <span class="nav-link text-white">{{ usersName }}</span>
          </li>
                  ...
                  <div class="py-1 d-flex">
                    ...
                    <div class="d-flex flex-column justify-content-center">
                      <h6 class="mb-1 text-sm font-weight-normal">
                        <span class="font-weight-bold">새로운 메세지</span> 친구로부터
                      </h6>
              ...
              <li class="mb-2">
                <a class="dropdown-item border-radius-md" href="javascript:;">
                  <div class="py-1 d-flex">
                    ...
                    <div class="d-flex flex-column justify-content-center">
                      <h6 class="mb-1 text-sm font-weight-normal">
                        <span class="font-weight-bold">새로운 앨범</span> by 최방문
                      </h6>
                      <p class="mb-0 text-xs text-secondary">
                        <i class="fa fa-clock me-1"></i>
                        1 일
...
export default {
  ...
  created() {
    console.log('Current usersName:', this.usersName); // usersName 값 확인
    this.minNav;
  },
  methods: {
    async makeLogin() {
      try {
        const response = await axios.post('/api/auth/signin', this.loginForm);
        this.$store.dispatch('login', response.data.token);
        this.$store.dispatch('fetchUserDetails'); // 사용자 정보 가져오기
